...
for (const mode of [
  ...
  test.describe(mode.label, () => {
    for (const route of mode.routes) {
      test(`${route.name} renders at current viewport`, async ({
        page,
      }, testInfo) => {
        const getConsoleErrors = attachConsoleErrorGuard(page);
        const authenticated = route.authRequired
          ? await loginFirstConfiguredRole(page)
          : false;
        ...
        await attachScreenshot(
          page,
          testInfo,
          `${testInfo.project.name}-${route.name}.png`,
        );

        expect(
          getConsoleErrors(),
          `${route.name} should not emit console or page errors`,
        ).toEqual([]);
      });
    }
  });
