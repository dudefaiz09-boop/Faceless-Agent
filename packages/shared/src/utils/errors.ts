...
export function formatError(error: any): AppError {
  if (error instanceof Error) {
    if (error.message.includes('401')) return { code: ErrorCode.UNAUTHORIZED, message: 'Session expired. Please log in again.' };
    if (error.message.includes('403')) return { code: ErrorCode.FORBIDDEN, message: 'You do not have permission for this action.' };
