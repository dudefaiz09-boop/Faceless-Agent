...
function materializeRows<T>(
  ...
  if (options.schoolId && collectionName !== 'schools') {
    records = records.filter((record) => {
      const data = record as Record<string, unknown>;
      return data.schoolId === options.schoolId || data.tenantId === options.schoolId;
    });
  }
...
export async function listDocuments<T>(
  ...
  return materializeRows<T>((data || []) as DocumentRow[], collectionName, options);
}
...
export function useDocuments<T>(collectionName: string, options: DocumentListOptions = {}) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mobile-documents:${collectionName}:${optionsKey}`)
      .on(
        'postgres_changes',
