...
import { 
  ...
  Baby, GraduationCap, Clock, AlertCircle
} from 'lucide-react';
import { cn } from '../lib/utils';
import { StudentProfile, AttendanceRecord, Assignment, Submission } from '@educonnect/shared-education';

export const ParentPortal = () => {
  const { user, linkedStudentIds } = useAuth();
  const [selectedStudentId, setSelectedStudentId] = useState<string | null>(linkedStudentIds[0] || null);
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      ...
      try {
        ...
        const profile = await apiClient.request<StudentProfile>(`/api/students/${selectedStudentId}`);
        setStudentData(profile);

        // Fetch attendance
        const att = await apiClient.request<AttendanceRecord[]>(`/api/attendance/history/${selectedStudentId}`);
        setAttendance(att);
        ...
        if (profile.classId) {
          ...
          const subs = await apiClient.request<Submission[]>(`/api/assignments/history/${selectedStudentId}`);
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Monitoring academic progress for your children.</p>
        </div>
           ...
           {linkedStudentIds.map((id) => (
             <button
               ...
               className={cn(
                 ...
                 selectedStudentId === id ? "bg-blue-600 text-white shadow-lg shadow-blue-200" : "text-slate-500 hover:bg-slate-200"
               )}
             >
               <Baby size={16} />
               {id === selectedStudentId && studentData?.displayName ? studentData.displayName : `Student ${id.substring(0, 4)}`}
             </button>
           ))}
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
           ...
           <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Syncing Dependent Records...</p>
        </div>
      ) : !selectedStudentId ? (
        <div className="bg-white p-20 rounded-[40px] text-center border border-slate-100 shadow-sm">
           ...
           <p className="text-slate-500 mt-2">Please contact the administration to link your account to your children.</p>
        </div>
      ) : (
          ...
          <div className="space-y-8">
            ...
            <div className="bg-indigo-600 p-8 rounded-[40px] text-white shadow-xl shadow-indigo-100 relative overflow-hidden">
               <div className="relative z-10">
                 <p className="text-indigo-100 text-xs font-black uppercase tracking-widest mb-2">Student Profile</p>
            ...
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
               <div className="flex items-center justify-between mb-6">
                 ...
                 <span className="bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase px-2 py-1 rounded-lg">Realtime</span>
               </div>
                  ...
                  {attendance.slice(0, 5).map((record) => (
                    <div key={record.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                       ...
                       <div className={cn(
                         ...
                         record.status === 'present' ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-500"
                       )}>
                         {record.status === 'present' ? <Clock size={18} /> : <AlertCircle size={18} />}
                       </div>
                    </div>
                  ))}
               </div>
             ...
             <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
                ...
                <div className="space-y-4">
                   {assignments.map((assignment) => {
                     ...
                     return (
                       <div key={assignment.id} className="group p-6 rounded-3xl border border-slate-100 hover:border-blue-100 transition-all bg-slate-50/50 hover:bg-white hover:shadow-xl hover:shadow-blue-900/5">
                          <div className="flex items-start justify-between mb-4">
                             <div className="space-y-1">
                                <h4 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{assignment.title}</h4>
                                <p className="text-xs text-slate-400 font-medium">Due: {assignment.dueDate}</p>
                             </div>
                             {submission ? (
                               <div className={cn(
                                 "px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                                 submission.status === 'graded' ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"
                               )}>
                                 {submission.status}
                               </div>
                             ) : (
                          ...
                          {submission?.status === 'graded' && (
                            <div className="bg-white p-4 rounded-2xl border border-slate-100 mt-4 flex items-center justify-between">
                               <div>
                                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Grade Received</p>
                               ...
                               <div className="text-right">
                                  ...
                                  <p className="text-sm text-slate-600 font-medium italic truncate max-w-[200px]">&quot;{submission.feedback}&quot;</p>
                               </div>
                            </div>
                          )}
                       </div>
